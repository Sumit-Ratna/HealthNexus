...
const Records = ({ viewingPatientId }) => {
    ...
    const fetchConnectedDoctors = async () => {
        try {
            ...
            const res = await axios.get('http://localhost:8000/api/connect/patient/doctors', {
    ...
    const fetchDocuments = async () => {
        try {
            const token = localStorage.getItem('accessToken');
            const res = await axios.get(`http://localhost:8000/api/documents/patient/${targetUserId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleShareUpdate = async (docId, doctorIds) => {
        try {
            await axios.patch(`http://localhost:8000/api/documents/${docId}/share`, {
