...
const PatientHistory = () => {
    ...
    const handleEditProfileSave = async () => {
        try {
            ...
            await axios.put(`http://localhost:8000/api/doctor/patients/${patient_id}/profile`,
    ...
    const fetchPatientHistory = async () => {
        try {
            const token = localStorage.getItem('accessToken');
            const res = await axios.get(`http://localhost:8000/api/connect/doctor/patient/${patient_id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleDelete = async (docId) => {
        if (window.confirm("Are you sure? This will remove the record from your portal. (Note: Only records you created will be permanently deleted; patient-shared reports will simply be unshared).")) {
            try {
                const token = localStorage.getItem('accessToken');
                const res = await axios.delete(`http://localhost:8000/api/documents/${docId}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
    ...
    const handleAnalyze = async (docId) => {
        ...
        try {
            const token = localStorage.getItem('accessToken');
            const res = await axios.post(`http://localhost:8000/api/documents/${docId}/analyze`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
