...
exports.verifyOtp = async (req, res) => {
    ...
    try {
        ...
        if (firebaseToken === 'FAKE_FIREBASE_TOKEN_12345' || firebaseToken?.startsWith('FAKE_')) {
            console.log("⚠️ Using Fake Firebase Token for Login");
...
exports.register = async (req, res) => {
    ...
    try {
        // 1. Verify Token
        if (firebaseToken === 'FAKE_FIREBASE_TOKEN_12345' || firebaseToken?.startsWith('FAKE_')) {
            console.log("⚠️ Using Fake Firebase Token for Registration");
        } else if (admin && admin.apps.length) {
            try {
